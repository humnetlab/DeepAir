...
class MLPNet_large(nn.Module):
    def __init__(self, input_size):
        ...
        self.layer1 = nn.Sequential(
            nn.Linear(input_size, 512), nn.LeakyReLU(), nn.Dropout(p=0.5, inplace=True)
        )
        self.layer2 = nn.Sequential(
            nn.Linear(512, 256), nn.LeakyReLU(), nn.Dropout(p=0.5, inplace=True)
        )
        self.layer3 = nn.Sequential(
            nn.Linear(256, 64), nn.LeakyReLU(), nn.Dropout(p=0.5, inplace=True)
        )
        self.layer4 = nn.Sequential(
            nn.Linear(64, 16), nn.LeakyReLU(), nn.Dropout(p=0.5, inplace=True)
        )
...
# CNN to pre-train embed Elevation LandCover and Fire
class situNet_ElevaLandFire(nn.Module):
    def __init__(self, args):
        super(situNet_ElevaLandFire, self).__init__()
        ...
        self.Elevation_embedding = SpatialEmbedding(out_dim=3)  # origin 20
        self.LandCover_embedding = urbanNet.BasemapNet(out_dim=3)
        self.Fire_embedding = urbanNet.BasemapNet(out_dim=3)

        # prediction module
        self.mlp = MLPNet_large(56)  # Elevation 3, LandCover 3, Fire 3
    ...
    def forward(self, attr, config):
        ...
        Fire_vec = self.Fire_embedding(attr["Fire"])
